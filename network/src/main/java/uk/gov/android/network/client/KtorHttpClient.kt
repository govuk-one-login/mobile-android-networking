...
constructor(
    ...
    override suspend fun request(request: ApiRequestV2): GenericHttpResponse = try {
        httpClient
            .request {
                url(request.url)
                method =
    ...
    override suspend fun makeAuthorisedRequest(apiRequest: ApiRequest, scope: String): ApiResponse =
    ...
    override suspend fun makeRequest(apiRequest: ApiRequest): ApiResponse = when (apiRequest) {
    ...
    private suspend fun makeGetRequest(apiRequest: ApiRequest.Get): ApiResponse = try {
        val response =
    ...
    private suspend fun makePostRequest(apiRequest: ApiRequest.Post<*>): ApiResponse = try {
        val response =
    ...
    private suspend fun makeFormRequest(apiRequest: ApiRequest.FormUrlEncoded): ApiResponse = try {
        val response =
            httpClient.post(apiRequest.url) {
                headers {
                    apiRequest.headers.forEach { header ->
...
internal fun ContentType.toKtorContentType(): KtorContentType = when (this) {
